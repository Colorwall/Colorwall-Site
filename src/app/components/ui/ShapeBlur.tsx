...
void main() {
    ...
    float sdf;
    if (VAR == 0) {
        // Automatically adapt rectangle bounds to the element's aspect ratio
        vec2 b = (u_resolution.xy / min(u_resolution.x, u_resolution.y)) * 0.5 * size;
        sdf = sdRoundRect(st0, b, roundness);
...
const ShapeBlur: FC<ShapeBlurProps> = ({
  ...
  variation = 0,
  pixelRatioProp = 2,
  shapeSize = 0.95, // Default 0.95 to leave a little margin
  roundness = 0.05,
  borderSize = 0.05,
  circleSize = 0.3,
  circleEdge = 0.5
