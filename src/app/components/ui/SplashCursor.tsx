...
export default function SplashCursor({
  ...
  useEffect(() => {
    ...
    const displayShaderSource = `
      ...
      void main () {
          vec3 c = texture2D(uTexture, vUv).rgb;
              ...
              // Exaggerate normals for a more distinct liquid surface
              vec3 n = normalize(vec3(dx * 2.0, dy * 2.0, length(texelSize) * 0.5));
              
              // Directional light from top-left
              vec3 l = normalize(vec3(-0.5, 0.5, 1.0));

              // Diffuse reflection
              float diffuse = clamp(dot(n, l) + 0.3, 0.6, 1.0);
              c *= diffuse;
              
              // Specular highlights (makes it look wet and shiny)
              vec3 v = vec3(0.0, 0.0, 1.0);
              vec3 h = normalize(l + v);
              float specular = pow(max(dot(n, h), 0.0), 80.0);
              
              c += vec3(1.0) * specular * 1.5;
          #endif
    ...
    // tracks the current animation frame request so cleanup can cancel it
    ...
    function updateFrame() {
      ...
      step(dt);
      render(null);
      // store the handle so the cleanup return can call cancelAnimationFrame
    ...
    // named references for all event handlers so cleanup can removeEventListener
    ...
    // kicks off the raf loop on the very first mouse interaction
    ...
    // kicks off the raf loop on the very first touch interaction
    ...
    // cleanup: stop the render loop, detach all listeners, release gpu resources
    return () => {
      ...
      // explicitly signal the browser to free the webgl context and all associated
      // gpu-side resources (textures, framebuffers, shader programs) immediately
